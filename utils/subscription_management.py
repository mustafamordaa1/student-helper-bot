...
async def handle_subscription_purchase(user_id, plan_details):
    ...
    database.execute_query(
        ...
        (
            plan_details["subscription_type"],
            plan_details["subscription_end_date"],
            user_id,
        ),
    )
    return True
...
async def get_subscription_details(user_id):
    ...
    subscription_details = database.get_data(
        "SELECT type_of_last_subscription, subscription_end_time FROM users WHERE telegram_id = ?",
        (user_id,),
    )
...
async def check_subscription(update: Update, context):
    ...
    ## FUTUER: Enabling the check subscription
    query = None
    if update.message:
    ...
    if subscription_data and subscription_data[0][0] is not None:
        subscription_end_time = datetime.strptime(
            subscription_data[0][0], "%Y-%m-%d %H:%M:%S"
        )
        if subscription_end_time > datetime.now():
            ...
            if update.message:
                await query.reply_text(
                    "انتهت صلاحية اشتراكك. يرجى الاشتراك للوصول إلى هذا القسم."
                )
            else:
        ...
        if update.message:
            await query.reply_text(
                "أنت لا تمتلك اشتراكًا. اشترك الآن للاستمتاع بميزات إضافية!"
            )
        else:
            await query.edit_message_text(
                "أنت لا تمتلك اشتراكًا. اشترك الآن للاستمتاع بميزات إضافية!"
...
def create_serial_code_files(serial_code_data=SERIAL_CODE_DATA):
    ...
    for code_prefix, data in serial_code_data.items():
        generate_serial_codes(
            100, code_prefix, data["filename"]
        )  # Generate 100 codes per type
